...
def getposition(node):
    ...
    if node_end != '':
        ...
        print(f'Illegal end value  (<{node_end}>) for {gav(node, "word")}/{gav(node, "lemma")}')
        result = -1
    return result
...
def getvclasses(match: SynTree, mwetree: SynTree, mwecomponents: List[SynTree],
    ...
    if match_hd is not None:
        ...
        if  match_hdpt == 'ww' and compoundsep in matchhdlemma:
            ...
            # find the svp
            svp_positions = [getposition(node) for node in mwecomponents if gav(node, 'rel') == 'svp']
        ...
        zichobjmwenode = find1(mwetree, './node[@rel="obj" and node[@rel="hd" and @lemma="zich"]]')
        sematchnode = find1(match, './node[@rel="se"')
        zichobjmatchnode = find1(mwetree, f'./node[@rel="obj"]/node[@rel="hd" and {zichallexpression}]]')
        ...
        iavfound = False
        iavposition = None
        iavmwenode = find1(mwetree, './node[@rel="pc" and @cat="pp" and count(node) = 1]/node[@rel="hd" and @pt="vz"]' )
        iavmatchnode = find1(match, './node[@rel="pc" and @cat="pp" ]/node[@rel="hd" and @pt="vz"]')
