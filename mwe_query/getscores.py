...
def getscores(resmwes: MWEDict , refmwes: MWEDict, idfunc=is_samemwe) -> Tuple[List[SentIdRPF1], RPF1]:
    ...
    for sentid in refmwes:
        ...
        if sentid in resmwes:
            ...
            intersection = getintersection(resmwes_sentid, refmwes_sentid, idfunc=idfunc)
            lintersection = len(intersection)
            sentidscore = getevalscores(len(resmwes_sentid), len(refmwes_sentid), lintersection)

            sentidscores.append((sentid, sentidscore, len(resmwes_sentid), len(refmwes_sentid),lintersection))
            ...
            sentidscores.append((sentid, sentidscore, 0, len(refmwes_sentid), 0))
    ...
    for sentid in resmwes:
        ...
        if sentid not in refmwes:
            ...
            sentidscores.append((sentid, sentidscore, len(resmwes_sentid), 0, 0))


    refmwescount = sum([len(refmwes[id]) for id in refmwes])
    resmwescount = sum([len(resmwes[id]) for id in resmwes])
    overallscore = getevalscores(resmwescount, refmwescount, len(overallintersection))
