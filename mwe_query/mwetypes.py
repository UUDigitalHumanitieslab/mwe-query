...
from sastadev.treebankfunctions import getattval as gav, clausecats, allcats, allpts, allrels
...
pt2idclass = {'n': 'NID', 'adj': 'AdjID', 'bw': 'AdvID', 'vnw': 'PronID', 'ww': 'VID',
              'vz': 'PID', 'vg': 'CID', 'tw': 'NID', 'tsw': 'NID', 'let': 'NID', 'lid': 'PronID', 'spec': 'NID'}

modrels = ['predm', 'mod', 'app', 'obcomp', 'me']

def getmweclasses(mwe: str, mwepos: str, annotations: List[int], headposition: int, mwecomponents: List[SynTree]) -> List[str]:
    results = []
    ...
    if mwepos in ['n']:
        ...
        if cheadposition >= 0 and annotations[cheadposition] in lvcannotation2annotationcodedict:
...
def ismvc(mwecomponents: List[SynTree]) -> bool:
    ...
    results2 = len([mwecomponent for mwecomponent in mwecomponents if gav(mwecomponent, 'wvorm') != 'inf']) <= 1
    result = result1 and results2
    return result

def getmwetype(mwematch: Optional[SynTree], mwepos: str, mweclasses:List[str]) -> Mwetype:
    '''
        ...
        if len(mweclasses) == 0:
            ...
            if theclass in ['NID', 'PronID']:
                ...
                if mwerel in {'mod', 'predm', 'me'}:
                    ...
                    result = pt2idclass[udheadlabel] if udheadlabel in pt2idclass else 'UID'
