...
from mwetyping import NodeCondition, Pos, Xpathexpression
...
def getprepprtsubnodes(node: SynTree,  alllemmanodes: List[SynTree]) -> SynTree:
    ...
    """
    subnodes = []
    alllemmapts = [(gav(alllemmanode, 'lemma'), gav(alllemmanode, 'pt')) for alllemmanode in alllemmanodes]
    vzlemmapts = [(lemma, pt) for (lemma, pt) in alllemmapts if pt == 'vz' and lemma in vzandprts]
...
def getrwqnode(node: SynTree, majorlemmanodes: List[SynTree], alllemmanodes: List[SynTree]) -> SynTree:
    if node.tag != 'node':
        newnode = ET.Element(node.tag, node.attrib)
        newchildren = [getrwqnode(child, majorlemmanodes, alllemmanodes) for child in node]
        newnode.extend(newchildren)
        result = newnode
    else:
        origavdict = {att: node.attrib[att] for att in ['lemma', 'pt'] if att in node.attrib}
        origsubnode = ET.Element('subnode', origavdict)
        otherlemmasubnodes = getotherlemmasubnodes(node)
        prepprtsubnodes = getprepprtsubnodes(node, alllemmanodes) if gav(node, 'pt') == 'ww' else []
