...
def mkpronadvs(prep: str, postp:str =None) -> List[str]:
    if prep == 'met':
        newprep = 'mee'
    elif prep == 'tot':
        newprep = 'toe'
...
advprons1 = set(rpronoun + radposition for rpronoun in Rpronouns for radposition in Radpositions)

advprons2 = set(rpronoun + ''.join(circumposition) for rpronoun in Rpronouns for circumposition in circumpositions)

pronadvlemmas = advprons1.union(advprons2)

robustadvprons1 = set(set(rpronoun + radposition for rpronoun in robustrpronouns for radposition in Radpositions))
robustadvprons2 = set(rpronoun + ''.join(circumposition) for rpronoun in robustrpronouns for circumposition in circumpositions)
...
def test():
    testadvprons = [('eraan', aanvz) , ('hieraan', aanvz), ('waaraan', aanvz), ('daaraan', aanvz),
                    ("d'raan", aanvz), ('draan', aanvz),
                    ('ermee', meevz), ('hiermee', meevz), ('waarmee', meevz), ('daarmee', meevz),
                    ("d'rmee", meevz), ('drmee', meevz),
                    ('ertoe', toevz), ('hiertoe', toevz), ('waartoe', toevz), ('daartoe', toevz),
                    ("d'rtoe", toevz), ('drtoe', toevz),
                    ('eropaf', opafvz), ('hieropaf', opafvz), ('waaropaf', opafvz), ('daaropaf', opafvz),
                    ("d'ropaf", opafvz), ('dropaf', opafvz)]
