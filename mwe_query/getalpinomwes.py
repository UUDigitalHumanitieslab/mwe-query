...
from sastadev.treebankfunctions import getattval as gav, getnodeyield, getsentence, terminal
...
def getalpinomwes(syntree: SynTree, sentenceid=None) -> List[MWEMeta]:
    mwemetas = []
    mwelexicon = 'Alpino'
    ...
    for verb in verbs:
                ...
                if isparticleverb(verb):
                    ...
                    if terminal(sibling):
                        ...
                        if mwuppok:
                            ...
                            siblingleaves = getnodeyield(sibling)
                            svpnodes = getsvpnodes(siblingleaves)
                            wwsvpnodecands = [svpnode for svpnode in svpnodes if gav(svpnode, 'pt') == 'ww']
                            if wwsvpnodecands != []:
                                wwsvpnode = wwsvpnodecands[0]
                                svpnodes = [svpnode for svpnode in svpnodes if svpnode != wwsvpnode]
                            else:
                                wwsvpnode = None
                            mwenodes += siblingleaves
                            if siblingcat in ['ti', 'inf']:
        ...
        if classes != []:
            ...
            mwemeta = MWEMeta(sentence, sentenceid, mweid, mwelexicon, mwequerytype, mweid, intpositions, headposition,
                              headpos, classes, parsemetype)
            mwemetas.append(mwemeta)
    vzs = syntree.xpath('.//node[@pt="vz" and @rel="hd" ]')  # no condition on vztype because of 'ergens op af'
    for vz in vzs:
        ...
        for az in vzazsiblings:
            ...
            mwemeta = MWEMeta(sentence, sentenceid, mweid, mwelexicon, mwequerytype, mweid, intpositions,
                              headposition, headpos, classes, parsemetype)
            mwemetas.append(mwemeta)
    return mwemetas
...
def ismwupp(node:SynTree) -> Tuple[bool, SynTree, List[SynTree]]:
    ...
    if nodecat == 'pp':
        ...
        if mwunode is not None and pcnode is not None and pcnodehdnode is not None and mwunodehdnode is not None:
