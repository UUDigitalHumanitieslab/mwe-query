...
mwu_exceptiondict = {'noem maar op': ('ww', 1),
                     'deo volente': ('ww', 2),
                     'nolens volens': ('ww', 2),
                     'alea iacta est': ('ww', 3),
                     'linea recta': ('bw', 1),
...
nameinfixes = {"'t",  'de', 'den', 'der', 'des', "'s", 'ten',  'ter', 'van',  }
...
def get_mwuprops(stree: SynTree) -> Tuple[SynTree, PosTag, int]:
    ...
    if result is None:
        print(f'Unknown mwu type: {mwuroot}: {comma.join([f"{gav(child, ptatt)}/{gav(child, lemmaatt)}" for child in stree])}')
        result = stree[0], 'n', 1
    return result
