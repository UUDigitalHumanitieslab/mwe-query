...
def tryannotate():
    ...
    sentences += [(1, 'hij poetste de plaat'), (2, 'hij poetste de mooie plaat'),
                  # (3, 'hij poetste, maar de plaat werd niet mooi') completely wrong parse so MEQ
                  (3, 'hij poetste, hoewel de plaat niet mooier werd')
                  ]
    sentences += [(4, 'Daar kraait geen haan naar'), (5, 'Hier heeft geen haan naar gekraaid'),
                  (6, 'geen haan kraaide daarnaar'), (7, 'geen haan kraaide ernaar dat hij niet kwam'),
                  (8, 'geen haan kraaide er naar dat hij niet kwam'),
                  (9, 'er is geen haan die daar naar kraait')]
    sentences += [(10, 'Een varkentje dat even vlug gewassen moest worden door PSV Eindhoven .')]
    sentences += [(11, 'als puntje bij paaltje komt laat hij het afweten')]
    sentences += [(12,'iemand zal als de kippen er bij zijn')]
    sentences += [(13, 'iemand zal een tik van de molen krijgen'), (14, 'iemand zal iemand tegenover zich krijgen')]
    sentences += [(15, 'De buurman van An houdt de boeken van Piet die zij houdt')]
    sentences += [(16, 'hij zal  als de kippen er bij zijn'), (17, 'hij zal er als de kippen bij zijn')]
    sentences += [(18, 'hij legde de boeken neer'), (19, 'hij heeft de boeken neergelegd')]
    sentences += [(20, 'Er kraaide geen haan naar dat Saab de boeken neer moest leggen')]
    ...
    wb = mkworkbook('MWEmetadata_tryannotate.xlsx', [mwemetaheader], fullrowlist, freeze_panes=(1, 0))
    wb.close()
