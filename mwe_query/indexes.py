...
from canonicalform import preprocess_MWE, transformtree, genvariants, expandnonheadwords, gettopnode
from mwuwordlemmas import mwuwordlemmadict
from pronadvs import mkpronadvs
...
def writetb(mwetreebank):
    ...
    fulltb.write(mwetreebankfullname, encoding="UTF8", xml_declaration=False,
                 pretty_print=True)
        ...
        for i, mwe in imwes:
            ...
            if mwestr in mwetreebank:
                ...
                if mwetree is not None:
                    ...
                    meta1 = etree.Element('meta', attrib={'type': 'text', 'name': 'origutt', 'value': mwestr})
                    meta2 = etree.Element('meta', attrib={'type': 'text', 'name': 'id', 'value': mweid})
            ...
            if mwetree is not None:
                ...
                if mweid not in mweid2id or forcedlemmaupdate:
                    ...
                    lemmas = [rawlemma for rawlemma in rawlemmas if rawlemma != 'zullen']
                    mweid2id[mweid] = i
                    id2mweid[i] = mweid
...
if os.path.exists(indexfullname):  # and older than the source file
    ...
    if len(indexlist) == 4:
        indexes = Indexes(indexlist[0], indexlist[1], indexlist[2], indexlist[3], mwetreesdict)
...
indexlist = [indexes.mweid2id, indexes.id2mweid, indexes.lemma2iddict, indexes.lemmasofmwedict]
